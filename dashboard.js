document.addEventListener('DOMContentLoaded', function() {
    ...
    // إضافة ذكر جديد
    document.getElementById('addDhikrForm').addEventListener('submit', function(e) {
        e.preventDefault();
    ...
    // تحديث الإعدادات
    document.getElementById('soundToggleSetting').addEventListener('change', function() {
    ...
    document.getElementById('passwordForm').addEventListener('submit', function(e) {
        ...
        if (currentPassword === "Rashed@2024") { // تحقق من كلمة المرور الحالية
        ...
        window.reportChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'التسبيحات',
                    data: [],
                    borderColor: getComputedStyle(document.documentElement).getPropertyValue('--dashboard-primary'),
                    tension: 0.4,
                    fill: true
                }]
            },
    ...
    window.generateReport = function() {
        ...
        const dailyAvg = Math.round(totalDhikr / (parseInt(localStorage.getItem('activeDays')) || 1));
    ...
    window.exportReport = function() {
        try {
            ...
            // إضافة التاريخ
            doc.setFontSize(12);
            doc.text(`التاريخ: ${new Date().toLocaleDateString('ar')}`, 105, 30, { align: 'center' });
            
            // إضافة الإحصائيات
            const data = [
                ['إجمالي التسبيحات', document.getElementById('reportTotal').textContent],
        ...
        window.myChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: `تقرير ${type === 'daily' ? 'يومي' : type === 'weekly' ? 'أسبوعي' : 'شهري'}`,
                        font: { size: 16 }
                    }
                }
            }
...
function updateDashboardStats() {
    ...
    document.getElementById('reportAverage').textContent = Math.round(totalCount / activeDays);
